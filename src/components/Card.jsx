...
const Card = ({ data, isApplicantHome }) => {
  const { _id, companyLogo, jobTitle, companyName, jobLocation, employmentType, minPrice, maxPrice, postingDate, description } = data;
  const linkTo = isApplicantHome ? `/applicant-job-details/${_id}` : `/jobs/${_id}`;

  return (
    <div>
      <section className="card">
        <Link to={linkTo} className="flex gap-4 flex-col sm:flex-row items-start">
